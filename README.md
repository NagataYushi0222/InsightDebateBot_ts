...
- `/imakita`
  `/join` で保持している直近5分のVC音声と、実行チャンネルの直近5分のチャットを3行で要約します。
- `/join`
  VCへ参加し、今北産業用に直近5分だけを循環録音します。定期分析は実行しません。
...
まず対象VC内で `/join` を実行します。以後Botは定期分析をせず、直近5分だけを循環録音します。途中参加者がテキストチャンネルで `/imakita` を実行すると、Gemini API キーを登録した実行者のキーで、VC音声と同チャンネルの直近5分のチャットをまとめて要約します。

- Bot の投稿、添付のみの投稿、空メッセージは要約対象外です
- 音声・チャットともに直近5分だけを対象にします
