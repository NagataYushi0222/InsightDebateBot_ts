...
- `/imakita`
  今北産業。実行したテキストチャンネルの直近最大50件の通常メッセージを、現在の話題・主な進捗・未決事項の3行で要約します。途中参加者の状況把握に使えます。
...
途中からテキストチャンネルへ参加した場合、そのチャンネルで `/imakita` を実行します。Gemini API キーを登録した実行者のキーで、直近の通常メッセージを要約してチャンネルへ投稿します。

- Bot の投稿、添付のみの投稿、空メッセージは要約対象外です
- 直近100件を取得し、そのうち最新50件までを対象にします
