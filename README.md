...
- `/analyze_stop`
  最終レポートを出さずに停止します。
- `/analyze_stop_final`
  実行中の定期処理があれば完了を待ち、最終レポートを作成してから停止します。
...
- `/article_start`
  VC 記事化用の録音を開始します。
- `/article_stop`
  録音を停止します。VC からは先に退出し、その後に記事候補トピックを抽出します。
- `/article_topics`
  直近のトピック候補を再表示します。
- `/article_archives`
...
- `/imakita`
  `/join` で保持している直近10分のVC音声と、実行チャンネルの直近10分のチャットを3行で要約します。
- `/join`
  VCへ参加し、今北産業用に直近10分だけを循環録音します。定期分析は実行しません。
  `/analyze_start` を実行した場合も、この録音は自動的に開始されるため、事前の `/join` は不要です。
- `/leave`
  今北産業用の循環録音を停止し、ほかの録音機能が動いていなければBotもVCから退出します。
...
分析時の検索は、Gemini の `googleSearch` ツールではなく、Bot 側の Web 検索関数を function calling で呼び出す方式です。
そのため、Gemini API 無料 Tier でモデルごとに Grounding の可否が異なる場合でも、検索付き分析を同じフローで使いやすくしています。

## 対応モデル

現状のコマンドから選べるモデルは次の 3 つです。`/settings set_model` または `/model` で、サーバーごとに切り替えられます。

- `gemini-3.8-flash`（既定）
  高い品質を優先する Flash モデルです。VC 音声を入力した要約・記事化にも対応します。
- `gemini-3.7-flash`
  3.8 の一世代前の Flash モデルです。既存の運用と品質・速度のバランスを保ちたい場合に選べます。
- `gemini-3.5-flash-lite`
  高頻度の要約や `/imakita` など、呼び出し回数・コストを優先する場合に向く軽量モデルです。

Gemini API の利用上限は API キーではなく Google Cloud プロジェクトの利用 Tier、リクエスト数、入力・出力トークン量で決まります。正確な RPM / TPM / RPD は Google AI Studio の Rate limits で確認してください。
...
推奨は `nohup` ではなく `systemd` で 1 プロセスだけ常駐させる運用です。
リポジトリには service 定義のサンプルとして `deploy/insight-vc-bot.service` を同梱しています。

```bash
sudo cp deploy/insight-vc-bot.service /etc/systemd/system/insight-vc-bot.service
sudo systemctl daemon-reload
sudo systemctl enable insight-vc-bot.service
sudo systemctl restart insight-vc-bot.service
```

状態確認:

```bash
systemctl status insight-vc-bot.service
tail -f bot.log
```
...
```bash
sudo systemctl stop insight-vc-bot.service
```

簡易運用例として `nohup` でも起動できます。

```bash
nohup bun run src/index_with_vc_article.ts > bot.log 2>&1 &
tail -f bot.log
```
...
```bash
pkill -f "bun run src/index_with_vc_article.ts"
```

## 動作の流れ

1. `/settings set_apikey` で API キーを保存
2. 必要に応じて `/model model:gemini-3.8-flash` などでモデルを選択
3. ボイスチャンネルに参加
4. `/analyze_start` で分析開始
5. 一定間隔でレポートが投稿される
6. 必要に応じて `/analyze_now` で手動分析
7. 終了時に `/analyze_stop` または `/analyze_stop_final`

レポートはテキストチャンネルに投稿され、本文はスレッドに分割送信されます。
`/analyze_stop_final` は、stop 直前に動いていた periodic analysis と競合しないよう、内部の分析ループが止まるのを待ってから最終レポートを生成します。

## 今北産業の使い方

まず対象VC内で `/join` を実行します。以後Botは定期分析をせず、直近10分だけを循環録音します。途中参加者がテキストチャンネルで `/imakita` を実行すると、Gemini API キーを登録した実行者のキーで、VC音声と同チャンネルの直近10分のチャットをまとめて要約します。

- Bot の投稿、添付のみの投稿、空メッセージは要約対象外です
- 音声・チャットともに直近10分だけを対象にします
- 要約には会話ログ由来の指示を実行しないよう保護を入れています
- 要約結果はチャンネル参加者に見える通常投稿です
...
4. 必要なら同じ VC で `/analyze_start` も実行し、リアルタイム要約と並行利用
5. 必要に応じて同じテキストチャンネルで会話ログも投稿
6. `/article_stop` で録音停止
7. Bot は VC から先に退出し、保存済み音声から記事候補を抽出
8. `/article_write topic:<番号>` で記事生成

記事化機能では、ユーザーごとの音声ファイルと、同じテキストチャンネルでの発言をまとめて Gemini に渡し、記事化向きの話題を抽出します。
`/article_stop` 後は録音を継続せず、保存済みファイルを使って AI 処理を進めるため、記事候補抽出中に VC に居座り続けないようになっています。
...
- `deploy/insight-vc-bot.service`
  Bun 版 Bot を `systemd` で 1 プロセス運用するための service 定義例です。
